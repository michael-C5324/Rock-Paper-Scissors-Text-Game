...
p = 0   #Number of times the player wins
c = 0   #Number of times the computer wins
  
def rps(player, comp):      #Sets conditions for who wins in each pairing
    global c
    global p
...
while True:
    ...
    i = random.randint(1,3)     #Defines value for the computers random choice
    if i == 1:
        h = "Rock"
    if i == 2:
        h = "Paper"   
    ...
    pick = str(input("Make your pick! "))       #Defines a value for user input choise
    if "Rock" in pick or "rock" in pick:
        ...
        x = "Scissors"    
    elif "No" in pick or "no" in pick or "Quit" in pick or "quit" in pick:      #Me being anal about the grammer output of total wins
        if c != 1 and p != 1:
